...
import { Document, Font, G, Page, Path, Svg, Text, View, StyleSheet, renderToBuffer } from '@react-pdf/renderer'
...
const s = StyleSheet.create({
  ...
  headerMeta: { fontFamily: 'GT Pressura Mono', fontWeight: 300, fontSize: 9, color: BLACK },
  fromTo: { fontFamily: 'GT Pressura Mono', fontWeight: 300, fontSize: 9, color: BLACK, lineHeight: 1.5 },
  // ── Cover ──
  coverTitle: {
    fontFamily: 'Thunder',
    fontSize: 100,
  ...
  redBar: { width: 110, height: 18, backgroundColor: RED, marginTop: 10, marginBottom: 36 },
  infoBox: { borderColor: RED, borderWidth: 1, borderStyle: 'solid' },
  infoBoxTop: {
    paddingVertical: 16,
  ...
  infoBoxBottom: { paddingVertical: 32, paddingHorizontal: 24, alignItems: 'center' },
  totalText: { fontFamily: 'Thunder', fontSize: 82, color: RED, lineHeight: 1 },
  // ── Overview ──
  overviewTitle: { fontFamily: 'Thunder', fontSize: 48, color: RED, lineHeight: 1, textAlign: 'right' },
  sectionTitle: {
    fontFamily: 'Thunder',
    fontSize: 60,
    color: BLACK,
  ...
  divider: { borderTopColor: RED, borderTopWidth: 1, borderTopStyle: 'solid', marginTop: 24 },
  // ── Bank ──
  bankTitle: { fontFamily: 'Thunder', fontSize: 82, color: RED, lineHeight: 1, marginTop: 36, marginBottom: 28 },
  bankSubtitle: { fontFamily: 'GT America', fontWeight: 700, fontSize: 10, color: BLACK, marginBottom: 20 },
  bankField: { marginBottom: 14 },
  bankLabel: { fontFamily: 'GT Pressura Mono', fontWeight: 300, fontSize: 9, color: RED, marginBottom: 2 },
  bankValue: { fontFamily: 'GT Pressura Mono', fontWeight: 300, fontSize: 10, color: BLACK, lineHeight: 1.6 },
  // ── Footer ──
  footer: {
    position: 'absolute',
    bottom: 28,
  ...
  footerEmail: { fontFamily: 'GT Pressura Mono', fontWeight: 300, fontSize: 9, color: RED },
  footerWeb: { fontFamily: 'GT Pressura Mono', fontWeight: 300, fontSize: 9, color: BLACK },
})

// SVG paths extracted from src/components/MarcopoloLogo.tsx
const LOGO_PATHS = [
...
const DocHeader = ({ date, client, rightText }: { date: string; client: string; rightText?: string }) => (
  <View>
    <View style={s.headerRow}>
      <Text style={s.headerMeta}>{date}</Text>
      <Text style={s.headerMeta}>{(rightText ?? client).toUpperCase()}</Text>
    </View>
    <Text style={s.fromTo}>FROM: MARCO POLO</Text>
    <Text style={s.fromTo}>TO: {client.toUpperCase()}</Text>
  </View>
)

const DocFooter = () => (
  <View style={s.footer}>
    <MarcopoloLogo color={RED} width={84} />
    <View style={s.footerRight}>
      <Text style={s.footerEmail}>hi@marcopolo.agency</Text>
      <Text style={s.footerWeb}>www.marcopolo.agency</Text>
    </View>
  </View>
)

const CoverPage = ({ client, amount, services, date }: Pick<DocProps, 'client' | 'amount' | 'services' | 'date'>) => (
  <Page size="A4" style={s.page}>
    <DocHeader date={date} client={client} />
    <Text style={s.coverTitle}>PAYMENT{'\n'}REQUEST</Text>
    <View style={s.redBar} />
    <View style={s.infoBox}>
      <View style={s.infoBoxTop}>
        <Text style={s.infoBoxTopText}>{services.toUpperCase()}</Text>
      </View>
      <View style={s.infoBoxBottom}>
        <Text style={s.totalText}>TOTAL: U$ {amount}</Text>
      </View>
    </View>
    <DocFooter />
  </Page>
...
const OverviewPage = ({ client, month, items, date }: Pick<DocProps, 'client' | 'month' | 'items' | 'date'>) => (
  <Page size="A4" style={s.page}>
    <View style={s.headerRow}>
      <View>
        <Text style={s.headerMeta}>{date}</Text>
      ...
      <Text style={s.overviewTitle}>{month.toUpperCase()}{'\n'}OVERVIEW</Text>
    </View>
    {Object.entries(items).map(([section, sectionItems]) => (
      <View key={section}>
        <Text style={s.sectionTitle}>{section.toUpperCase()}</Text>
        {sectionItems.map((item, i) => (
          <Text key={i} style={s.sectionItem}>{item}</Text>
        ))}
      </View>
    ))}
...
const BANK_FIELDS: [string, string][] = [
  ['Account Holder', 'Mateo Aldao Suaya'],
  ['Account number', '8312071894'],
...
const BankPage = ({ client, date }: Pick<DocProps, 'client' | 'date'>) => (
  <Page size="A4" style={s.page}>
    <DocHeader date={date} client={client} />
    <Text style={s.bankTitle}>BANK{'\n'}DETAILS</Text>
    <Text style={s.bankSubtitle}>USD account details.</Text>
    {BANK_FIELDS.map(([label, value]) => (
      <View key={label} style={s.bankField}>
...
const PaymentRequestDoc = (props: DocProps) => {
  const hasItems = Object.keys(props.items).length > 0
  return (
    <Document>
      <CoverPage client={props.client} amount={props.amount} services={props.services} date={props.date} />
      {hasItems && (
        <OverviewPage client={props.client} month={props.month} items={props.items} date={props.date} />
      )}
      <BankPage client={props.client} date={props.date} />
    </Document>
  )
}
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const { client, amount, month, services, items, date } = req.body ?? {}
  ...
  const resolvedDate: string = date ?? `${today.getDate()} | ${today.getMonth() + 1} | ${today.getFullYear()}`
  const resolvedAmount: string = typeof amount === 'number'
    ? amount.toLocaleString('de-DE')
    : String(amount)
  const resolvedServices: string = services ?? 'PLATFORM UX UI / BRANDING / DEVELOPMENT'
  const resolvedItems: Record<string, string[]> = items ?? {}
  ...
  const filename = `Marco-Polo-Payment-Request-${client}-${month}.pdf`
