...
const CLIENT_NAMES: Record<string, string> = {
  '30714015377': 'HEKTOR S.R.L.',
  '20330327562': 'CARRIQUIRI IGNACIO FEDERICO',
  '30716140403': 'MOSERINI SAS',
}
...
const BAR_SCALE = CATS[2].limit // scale to Cat C
...
export default function Monotributo() {
  ...
  useEffect(() => {
    fetch('/api/invoices')
      .then(r => r.json())
      .then(data => {
        const mapped: Invoice[] = (data.invoices ?? []).map((inv: { num: string; date: string; cuit: string; amount: number }) => ({
          num: inv.num,
          date: inv.date,
          client: CLIENT_NAMES[inv.cuit] ?? inv.cuit,
          amount: inv.amount,
        }))
        setInvoices(mapped)
        setLoading(false)
      ...
      .catch(() => {
        setError('No se pudieron cargar las facturas desde ARCA.')
        setLoading(false)
      })
  }, [])

  const baseTotal = useMemo(() => invoices.reduce((s, i) => s + i.amount, 0), [invoices])
  ...
  // Average: unique billed months, based on base total (not simulated)
  const avgMonthly = useMemo(() => {
    if (!invoices.length) return 0
    const months = new Set(invoices.map(i => i.date.slice(0, 7))).size
    return baseTotal / Math.max(months, 1)
  }, [invoices, baseTotal])
  ...
  const monthsToNext = avgMonthly > 0 ? remainingToNext / avgMonthly : 0
    ...
    <AuthGate>
      <div className="min-h-screen bg-white">
        ...
        <main className="px-6 py-12 md:px-[8vw] max-w-[860px] mx-auto space-y-14">
          ...
          {/* Loading / error */}
          {loading && (
            <div className="py-20 text-center text-sm text-black/30 animate-pulse">
              Consultando ARCA…
            </div>
          )}

          {error && (
            <div className="py-6 text-center text-sm text-red-500 border border-red-200 rounded-xl">
              {error}
            </div>
          )}
            ...
            <>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                <StatCard
                  ...
                  value={money(total)}
                  sub={nextAmount > 0 ? `base ${money(baseTotal)} + ${money(nextAmount)} simulado` : `${invoices.length} facturas`}
                  accent
                />
                <StatCard
                  label="Promedio / mes"
                  value={money(Math.round(avgMonthly))}
                  sub={`${new Set(invoices.map(i => i.date.slice(0, 7))).size} meses`}
                />
                <StatCard
              ...
              <section>
                ...
                <div className="relative h-6 mb-1">
                  {CATS.slice(0, 3).map((c) => {
                    ...
                    if (p > 97) return null
                    return (
                      <div key={c.cat} className={`absolute text-[11px] font-bold -translate-x-1/2 ${CAT_TEXT[c.cat]}`} style={{ left: `${p}%` }}>
                        Cat {c.cat}
                      </div>
                    )
                  })}
                ...
                <div className="relative h-4">
                  {CATS.slice(0, 3).map((c) => {
                    ...
                    if (p > 97) return null
                    return (
                      <div key={c.cat} className="absolute text-[10px] text-black/30 -translate-x-1/2 whitespace-nowrap" style={{ left: `${p}%` }}>
                        {money(c.limit)}
                      </div>
                    )
                  })}
              ...
              {/* Monthly projection */}
              <section>
                <h2 className="font-thunder text-2xl uppercase mb-1">Proyección mensual</h2>
              ...
              <section className="border border-[var(--marco-border)] rounded-2xl p-6 md:p-8">
                ...
                <p className="text-xs text-black/40 mb-6">El dashboard se actualiza en tiempo real</p>
                <div className="flex flex-wrap gap-4 items-center mb-6">
                  ...
                  {nextAmount > 0 && (
                    <button onClick={() => setNextRaw('')} className="text-xs text-black/30 hover:text-black/60 underline">
                      Limpiar
                    </button>
                  )}
                </div>
                {nextAmount > 0 && (
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                    ...
                    />
                    <SimResult label="Categoría" value={`Cat ${currentCat.cat}`} color={CAT_TEXT[currentCat.cat]} />
                    <SimResult
              ...
              <section>
                <div className="flex items-baseline justify-between mb-5">
                  <h2 className="font-thunder text-2xl uppercase">Historial 2026</h2>
                  <button
                    onClick={() => {
                      setLoading(true)
                      setError(null)
                      fetch('/api/invoices')
                        .then(r => r.json())
                        .then(data => {
                          setInvoices((data.invoices ?? []).map((inv: { num: string; date: string; cuit: string; amount: number }) => ({
                        ...
                        .catch(() => { setError('Error al recargar.'); setLoading(false) })
                    }}
                    ...
                    <thead>
                      <tr className="border-b border-[var(--marco-border)]">
                        {['Comprobante', 'Fecha', 'Cliente', 'Importe'].map((h, i) => (
                          <th key={h} className={`py-2 pr-4 text-[11px] text-black/30 font-normal ${i === 3 ? 'text-right pr-0' : 'text-left'}`}>
                            {h}
                          </th>
                        ))}
                      </tr>
...
function StatCard({ label, value, sub, accent, catColor }: {
...
function ProjectionRow({ label, projected, prevTotal, cat, crossed, isFirst }: {
  ...
  return (
    <div className="flex items-center gap-3">
      ...
      <div className="flex-1 h-5 bg-black/[0.04] rounded-full overflow-hidden relative">
        <div className="absolute top-0 left-0 h-full bg-black/10 rounded-l-full" style={{ width: `${fillPrev}%` }} />
        <div
        ...
        {CATS.slice(0, 3).map((c) => {
          const p = pct(c.limit)
          if (p > 99) return null
          return <div key={c.cat} className="absolute top-0 bottom-0 w-px opacity-60" style={{ left: `${p}%`, backgroundColor: CAT_BORDER[c.cat] }} />
        })}
      </div>
      <div className="w-28 flex-shrink-0 text-right">
        <span className={`font-thunder text-sm ${isFirst ? 'text-black' : 'text-black/60'}`}>{money(projected)}</span>
        {crossed && <span className={`ml-1.5 text-[9px] font-bold uppercase ${CAT_TEXT[crossed]}`}>↑ Cat {crossed}</span>}
      </div>
    </div>
  )
}
